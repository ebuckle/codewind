...
async function TrimLocalPackages(packageJson) {
...
async function CompareResultsWithExistingClearance(acceptedPackages, unclearedPackages, licenseCheckerOutput) {
    ...
    for (const key in licenseCheckerOutput) {
        const packageNameAndVersion = key.split('@', 2);
        const packageName = packageNameAndVersion[0];
        const packageVersion = packageNameAndVersion[1];
